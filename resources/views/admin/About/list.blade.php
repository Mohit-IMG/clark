                                        ...
                                        <table class="table table-hover js-basic-example contact_list dataTable"
                                            ...
                                            <thead>
                                                <tr role="row">
                                                    ...
                                                    <th class="center sorting" tabindex="0"
                                                        aria-controls="DataTables_Table_0" rowspan="1" colspan="1"
                                                        style="width: 126.333px;"
                                                        aria-label=" Name : activate to sort column ascending"> CV
                                                    </th> 
                                                    <th class="center sorting" tabindex="0"
                                                        aria-controls="DataTables_Table_0" rowspan="1" colspan="1"
												...
												@if(!empty($result))
													@foreach($result as $key=>$value)
														<tr class="gradeX odd"  id="{{ $value->id }}">
                                                            ...
                                                            <td class="center">{{ $value['profile'] }}</td>
                                                            <td class="center">{{ ucfirst($value['short_description']) }}</td>
                                                            <td class="center">
                                                                <embed src="{{ asset('uploads/cv/'.$value->cv) }}" width="200" height="200" type="application/pdf">
                                                            </td>
															<td class="center">
                                            ...
                                            <tfoot>
                                                <tr>
                                                    ...
                                                    <th class="center" rowspan="1" colspan="1"> CV </th>	
...
@push('custom_js')
    <script>
        ...
        $('.-change').change(function() {
            ...
            $.ajax({
                type: "POST",
                dataType: "json",
                url: "{{ route('admin.about.changestatus') }}",
				headers: {
    ...
    <script type="text/javascript">
        ...
        function updateOrder(aData) {        
            $.ajax({
                url: "{{ route('admin.about.change-order') }}",
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                type: 'POST',
